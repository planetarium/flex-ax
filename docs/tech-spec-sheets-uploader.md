...
flex-cli `crawl`가 수집한 워크플로우 JSON 데이터(양식 템플릿, 결재 문서 인스턴스, 근태/휴가 승인 이력)를 스프레드시트 형태로 변환하여 Google Sheets에 업로드하는 CLI 앱을 설계한다.

- flex-cli `crawl`의 `output/` 디렉토리를 입력으로 받아 JSON 파일을 읽는다
...
- flex-cli `crawl` 명령 구현: `apps/flex-cli/src/commands/crawl.ts`
...
| `zod` | 설정값 및 입력 JSON의 런타임 스키마 검증 | flex-cli `crawl`에서 이미 사용 중인 패키지. 외부 소스 데이터의 유효성을 보장한다 |
| `dotenv` | 환경 변수 로딩 | flex-cli `crawl` 컨벤션과 동일. 인증 정보 경로 등 설정을 `.env`에서 로딩한다 |
| `tsx` | TypeScript 직접 실행 | flex-cli `crawl` 컨벤션과 동일. 빌드 없이 `tsx src/main.ts`로 실행한다 |
...
flex-cli `crawl`와 동일한 **파이프라인 패턴**을 사용하여 단계를 순차 실행한다. 각 단계는 독립 모듈로 분리되며, 공통 인프라(설정, 로깅)를 공유한다.
...
### 3.4 flex-cli `crawl`와의 관계

flex-sheets-uploader는 flex-cli `crawl`에 대한 **런타임 의존성을 갖지 않는다**. flex-cli `crawl`의 출력 결과물(JSON 파일)만을 입력으로 사용한다. 따라서:

- flex-cli `crawl`의 타입 정의를 직접 import하지 않는다
- 입력 JSON의 구조를 `types/`에 별도로 정의하고 zod 스키마로 검증한다
- flex-cli `crawl`의 타입 변경 시 영향을 최소화한다
...
│   │   └── index.ts                # 로깅 (콘솔 기반)
│   └── types/
│       ├── input.ts                # 입력 JSON 타입 (flex-cli `crawl` output 구조)
...
- `reader/`: flex-cli `crawl`의 `storage/`(저장)에 대응하는 역방향 모듈. 파일 시스템에서 JSON을 읽고 파싱한다
- `transformer/`: 도메인별 변환 로직 분리. flex-cli `crawl`의 `crawlers/`가 도메인별 파일로 분리된 패턴을 따른다
- `sheets/`: 외부 서비스(Google Sheets) 통신을 단일 모듈로 격리. flex-cli `crawl`의 `auth/`(Playwright)에 대응한다
...
interface UploaderConfig {
  /** flex-cli `crawl` output 디렉토리 경로 (필수) */
  inputPath: string;
...
설정 로딩은 flex-cli `crawl`의 `config/index.ts` 패턴을 따른다:
...
flex-cli `crawl`의 `Logger` 인터페이스를 간소화한 버전이다. `progress`와 `summary`는 본 앱의 특성(처리 항목이 적음)에 맞게 `info`로 대체한다.

## 6. 상태 관리

### 6.1 상태 구조

flex-cli `crawl`와 동일하게, 일회성 파이프라인이므로 **함수 인자와 반환값**으로 상태를 전달한다.
...
현재 규모(템플릿 29건, 인스턴스 7건)에서는 전체 데이터를 메모리에 로딩해도 문제가 없다. flex-cli `crawl`의 스트리밍 저장 방식과 달리, 본 앱은 **읽기 → 변환 → 업로드** 순서로 전체 데이터를 메모리에 유지한다.
...
- **완료 기준**: flex-cli `crawl`의 output 디렉토리를 입력으로 받아 `ReadResult`를 반환한다. 유효하지 않은 JSON이 포함되어도 나머지를 정상 처리한다
...
- **공유 타입 패키지 추출**: flex-cli `crawl`와 입력 JSON 타입이 동일하므로, 향후 `packages/flex-types` 등으로 공통 타입을 추출할 수 있다
...
flex-cli `crawl`의 스크립트 컨벤션을 따른다. monorepo의 `turbo.json`에 정의된 `build`, `lint`, `clean` 태스크와 호환된다 (FR-019).

### 11.6 .env.example

```bash
# flex-cli `crawl` output 디렉토리 경로 (필수)
INPUT_PATH=../flex-cli `crawl`/output
