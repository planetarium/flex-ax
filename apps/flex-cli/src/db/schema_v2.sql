...
-- ============================================================
-- 부서 (트리 구조)
-- 엔드포인트: departments-search, user-employee-bundle,
--            user-personnel-appointments, search-users
-- ============================================================
...
-- ============================================================
-- 징계 종류 마스터
-- 엔드포인트: customer-disciplines
-- ============================================================
CREATE TABLE IF NOT EXISTS discipline_types (
  id            TEXT PRIMARY KEY,                -- disciplineIdHash
  customer_id   TEXT NOT NULL REFERENCES customers(id),
...
-- ============================================================
-- 직원 (HR 인사 정보)
-- 엔드포인트: user-employee, user-employee-bundle
-- ============================================================
...
-- ============================================================
-- 직원 직위 배정 (N:M: 한 직원이 복수 부서·직함 가능)
-- 엔드포인트: user-employee-bundle, user-personnel-appointments
-- ============================================================
CREATE TABLE IF NOT EXISTS user_positions (
  id                                TEXT PRIMARY KEY,  -- position idHash
  ...
  personnel_appointment_creation_id TEXT               -- 인사발령 생성 ID
);
...
CREATE TABLE IF NOT EXISTS user_job_roles (
  id                                TEXT PRIMARY KEY,  -- assignment idHash
  user_id                           TEXT NOT NULL REFERENCES users(id),
  customer_id                       TEXT NOT NULL REFERENCES customers(id),
...
-- ============================================================
-- 직원 직급 배정
-- 엔드포인트: user-employee-bundle (jobRanks 배열)
-- ============================================================
...
-- ============================================================
-- 급여 계좌
-- 엔드포인트: user-personal-bundle (payrollBankAccount)
-- ============================================================
...
-- ============================================================
-- 고용 계약
-- 엔드포인트: employment-contracts-search
-- ============================================================
CREATE TABLE IF NOT EXISTS employment_contracts (
  id                TEXT PRIMARY KEY,           -- employment.idHash
  user_id           TEXT NOT NULL REFERENCES users(id),
  customer_id       TEXT NOT NULL REFERENCES customers(id),
...
-- ============================================================
-- 인사발령
-- 엔드포인트: user-personnel-appointments
-- ============================================================
CREATE TABLE IF NOT EXISTS personnel_appointments (
  id                  TEXT PRIMARY KEY,         -- personnelAppointmentIdHash
  customer_id         TEXT NOT NULL REFERENCES customers(id),
...
-- ============================================================
-- 퇴직 기록
-- 엔드포인트: user-resignations (현재 빈 배열이나 스키마 존재)
-- ============================================================
CREATE TABLE IF NOT EXISTS user_resignations (
  ...
  type            TEXT,                         -- VOLUNTARY, INVOLUNTARY, …
  reason          TEXT,
  status          TEXT,
  raw             TEXT                          -- [PG] → JSONB
);
...
-- ============================================================
-- 부양가족
-- 엔드포인트: dependent-families-search
-- ============================================================
...
-- ============================================================
-- 학력 사항
-- 엔드포인트: education-experiences-search
-- ============================================================
...
-- ============================================================
-- 포상
-- 엔드포인트: user-rewards-search (현재 빈 배열이나 스키마 존재)
-- ============================================================
...
-- ============================================================
-- 징계 이력
-- 엔드포인트: user-disciplines-search (현재 빈 배열이나 스키마 존재)
-- ============================================================
...
-- ============================================================
-- 근무 규칙 (회사 단위)
-- 엔드포인트: work-rules-customer
-- ============================================================
CREATE TABLE IF NOT EXISTS work_rules (
  id                        TEXT PRIMARY KEY,   -- customerWorkRuleId (TEXT cast)
  customer_id               TEXT NOT NULL REFERENCES customers(id),
  rule_name                 TEXT,               -- "2023-09-01"
  ...
  calculation_strategy      TEXT,               -- FLEXIBLE_CALENDAR_DAY_BASED, …
  is_primary                INTEGER DEFAULT 0,  -- [PG] → BOOLEAN
  work_record_rule_id       TEXT,               -- customerWorkRecordRuleId
  week_working_hour_rule    TEXT,               -- [PG] → JSONB, 7일 상세 배열
  raw                       TEXT                -- [PG] → JSONB
);
...
CREATE TABLE IF NOT EXISTS work_forms (
  id               TEXT PRIMARY KEY,            -- customerWorkFormId (TEXT cast)
  customer_id      TEXT NOT NULL REFERENCES customers(id),
  name             TEXT NOT NULL,
  description      TEXT,
...
-- ============================================================
-- 근무 계획 (자동 주간 플랜)
-- 엔드포인트: work-plan-auto
-- ============================================================
CREATE TABLE IF NOT EXISTS work_plan_auto (
  user_id      TEXT NOT NULL REFERENCES users(id),
  day_of_week  TEXT NOT NULL,                   -- MONDAY … SUNDAY
  time_blocks  TEXT NOT NULL DEFAULT '[]',      -- [PG] → JSONB, [{workFormId, from, to}]
  PRIMARY KEY (user_id, day_of_week)
);

-- ============================================================
-- 근무 승인 규칙 (휴일근무 등)
-- 엔드포인트: work-approval-rules
-- ============================================================
CREATE TABLE IF NOT EXISTS work_approval_rules (
  id               TEXT PRIMARY KEY,            -- customerWorkApprovalRuleId (TEXT cast)
  customer_id      TEXT NOT NULL REFERENCES customers(id),
...
-- ============================================================
-- 휴가 정책 (time-off forms)
-- 엔드포인트: time-off-forms, time-off-policy-detail
-- ============================================================
CREATE TABLE IF NOT EXISTS time_off_policies (
  id                   TEXT PRIMARY KEY,        -- timeOffPolicyId (TEXT)
  customer_id          TEXT NOT NULL REFERENCES customers(id),
  ...
  assign_method        TEXT,                    -- ADMIN, MONTHLY, SERVICE_YEARS
...
-- ============================================================
-- 휴가 버킷 (연도별 할당 잔여량)
-- 엔드포인트: time-off-buckets
-- ============================================================
CREATE TABLE IF NOT EXISTS time_off_buckets (
  user_id              TEXT NOT NULL REFERENCES users(id),
  policy_id            TEXT NOT NULL REFERENCES time_off_policies(id),
  assigned_at          TEXT NOT NULL,           -- [PG] → TIMESTAMPTZ, 부여 시점
  valid_usage_from     TEXT,                    -- [PG] → DATE
  valid_usage_to       TEXT,                    -- [PG] → DATE
  assigned_minutes     INTEGER DEFAULT 0,       -- assignedTime (분)
  used_minutes         INTEGER DEFAULT 0,       -- usedTime (분)
  remaining_minutes    INTEGER DEFAULT 0,       -- remainingTime.timeOffMinutes
  expiration_date      TEXT,                    -- [PG] → DATE
  assign_method        TEXT,                    -- ADMIN, MONTHLY, …
...
-- ============================================================
-- 휴가 신청 태스크 (결재 단위)
-- 엔드포인트: time-off-approval-task
-- ============================================================
CREATE TABLE IF NOT EXISTS time_off_tasks (
  task_key          TEXT PRIMARY KEY,           -- taskKey (ULID 형태)
  user_id           TEXT NOT NULL REFERENCES users(id),
  ...
  content_units     TEXT DEFAULT '[]',          -- [PG] → JSONB, 정책별 사용 시간
  raw               TEXT                        -- [PG] → JSONB
);
...
-- ============================================================
-- 공휴일 그룹
-- 엔드포인트: holiday-groups, holiday-user-groups
-- ============================================================
CREATE TABLE IF NOT EXISTS holiday_groups (
  id              TEXT PRIMARY KEY,             -- customerHolidayGroupIdHash
  customer_id     TEXT NOT NULL REFERENCES customers(id),
  name            TEXT NOT NULL,
...
-- ============================================================
-- 캘린더 (개인 캘린더 메타)
-- 엔드포인트: calendar-primary, calendar-coworkers
-- ============================================================
CREATE TABLE IF NOT EXISTS calendars (
  id            TEXT PRIMARY KEY,               -- calendar token/id
  user_id       TEXT NOT NULL REFERENCES users(id),
  customer_id   TEXT NOT NULL REFERENCES customers(id),
  summary       TEXT,
...
CREATE TABLE IF NOT EXISTS todos (
  ...
  type             TEXT,                        -- WRITING_DOCUMENT_REQUEST, …
  title            TEXT,
  preview_content  TEXT,
  status           TEXT NOT NULL,               -- TODO, IN_PROGRESS, DONE
  reference_type   TEXT,                        -- APPROVAL_DOCUMENT_TEMPLATE
  reference_key    TEXT,
...
-- ============================================================
-- 피드백 / 칭찬 (Recognition & Feedback)
-- 엔드포인트: feedback-chunk, feedback-count
-- ============================================================
...
-- 피드백 수신자 (COMPANY 전체, 특정 부서, 특정 개인)
CREATE TABLE IF NOT EXISTS feedback_receivers (
  ...
  target_id   TEXT,                             -- departmentIdHash or userIdHash, NULL if COMPANY
  name        TEXT,
...
-- 엔드포인트: customer-attachments-search
-- ============================================================
...
-- ============================================================
-- 이해관계자 사용자 목록 (결재선 팝업 등에서 사용)
-- 엔드포인트: stakeholder-users
-- 참고: users 테이블에서 커버 가능하나 별도 뷰/스냅샷으로 관리
-- ============================================================
-- stakeholder-users 는 users 테이블 별칭으로 충분 (별도 테이블 불필요)
-- uuid → users.id, name → users.name 으로 매핑

-- ============================================================
-- 구독 기능 (활성화된 feature key 목록)
-- 엔드포인트: subscription-features
-- ============================================================
